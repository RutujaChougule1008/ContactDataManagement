...
const EventGroupMaster = () => {
    ...
    const handleSaveOrUpdate = () => {
        if (isEditMode) {
            axios.put(`${API_URL}/update-eventgroup/${newEventCode}`, formData)
                .then(response => {
            ...
            axios.post(`${API_URL}/create-eventgroup`, formData)
                .then(response => {
    ...
    const handleDelete = () => {
        if (window.confirm("Are you sure you want to delete this event?")) {
            axios.delete(`${API_URL}/delete-eventgroup?event_code=${newEventCode}`)
                .then(() => {
    ...
    const handleFirstButtonClick = () => {
      axios.get(`${API_URL}/get-first-eventgroup`)
          .then(response => {
  ...
  const handlePreviousButtonClick = () => {
      axios.get(`${API_URL}/get-previous-eventgroup?current_code=${newEventCode}`)
          .then(response => {
              setFormData(response.data);
              newEventCode = response.data.eventCode
              setIsEditMode(false);
  ...
  const handleNextButtonClick = () => {
      axios.get(`${API_URL}/get-next-eventgroup?current_code=${newEventCode}`)
          .then(response => {
              setFormData(response.data);
              newEventCode = response.data.eventCode
              setIsEditMode(false);
  ...
  const handleLastButtonClick = () => {
      axios.get(`${API_URL}/get-last-eventgroup`)
          .then(response => {
              setFormData(response.data);
              newEventCode = response.data.eventCode
              setIsEditMode(false);
